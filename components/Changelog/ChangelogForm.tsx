...
const formSchema = changelogEntrySchema.omit({
  id: true,
}).extend({
  details: changelogEntrySchema.shape.details.element.extend({
    id: z.string(),
  }).array(),
})
...
export default function ChangelogForm({ data }: Props) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    client.changelog.upsertChangelogEntry({
      id: data?.id,
      date: values.date,
      title: values.title,
      ...
      toast.error("Failed to save changelog entry", { id: uuid, description: error.message });
    });
  }
            ...
            <FormItem>
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => detailsField.onChange([...detailsField.value, { description: "", type: "", id: crypto.randomUUID() }])}
                >
                  Add Detail
                </Button>
              </div>
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem className="grid grid-cols-[1fr_4fr_max-content] gap-2 items-center">
                      <Select onValueChange={(v) => form.setValue(`details.${index}.type`, v as ChangelogDetailType)} defaultValue={detail.type}>
                        <FormControl>
                        ...
                        onClick={async () => {
                          try {
                            const dec = await showAlert({
                              title: "Remove Detail",
                              description: "Are you sure you want to remove this detail?",
                              confirmButton: {
                                text: "Remove",
                              },
                              cancelButton: {
                            ...
                            if (!dec) return;
                            form.setValue(`details`, detailsField.value.filter((f) => f.id !== detail.id));
                          } catch (error) {
                            console.error(error);
                          }
                        }}
