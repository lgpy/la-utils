    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="lg:min-w-5xl">
        ...
        {isLoading ? (
          ...
          <ScrollArea className="max-h- p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.map((character) => (
                <div key={character.id} className="p-4 border rounded-md space-y-2">
                  ...
                  <ul className="space-y-2">
                    {character.assignedRaids.map((raid) => (
                      <li key={raid.id} className="flex items-center justify-between">
                        <span>{raid.name}</span>
                        <Checkbox
                          checked={!raid.ignore}
