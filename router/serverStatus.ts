...
async function fetchServerStatus() {
  const response = await fetch("https://www.playlostark.com/en-us/support/server-status");
  if (!response.ok) {
...
function isStatusOld(updatedAt: Date, status: ServerStatus): boolean {
  ...
  const pollingInterval = isOfflineOrMaintenance ? POLLING_INTERVALS.FAST : POLLING_INTERVALS.SLOW;
...
export const getServerStatus = os
  ...
  .handler(async ({ context: { db }, input }) => {
    const isServerNameValid = Object.values(ServerName).includes(input as ServerName);
    ...
    if (serverStatus === null || isStatusOld(serverStatus.updatedAt, serverStatus.status)) {
