...
class StoneGame {
	...
	facet(line: number) {
		...
		const firstPendingIndex = row.findIndex((status) => status === "pending");
...
function runSimulation(iterations: number): WorkerResult {
	...
	for (let i = 0; i < iterations; i++) {
		...
		while (true) {
			const move = optimizer.getOptimalMove(
				new StoneState({
					line1: game.r1.map((status, idx) => ({
						detectedStatus: status,
						pos: idx,
					})),
					line2: game.r2.map((status, idx) => ({
						detectedStatus: status,
						pos: idx,
					})),
					line3: game.r3.map((status, idx) => ({
						detectedStatus: status,
						pos: idx,
					})),
...
if (!isMainThread) {
	...
	async function main() {
		...
		if (remainingIterations > 0) {
			console.log(
				`Main thread will run additional ${remainingIterations} iterations`
			);
		}
		...
		try {
			...
			const totalSuccessCount =
				results.reduce((sum, result) => sum + result.successCount, 0) +
				mainThreadResult.successCount;
			const totalIterations =
				results.reduce((sum, result) => sum + result.iterations, 0) +
				mainThreadResult.iterations;

			console.timeEnd("Total execution time");
			console.log(
				`Total games succeeded: ${totalSuccessCount} out of ${totalIterations}`
			);
			console.log(
				`Success rate: ${(totalSuccessCount / totalIterations) * 100}%`
			);
		} catch (error) {
