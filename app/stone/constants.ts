...
import { PixelCoordinate, PixelTarget } from './types';
...
export const generateLinePositions = (baseX: number, y: number, count: number, spacing: number): PixelCoordinate[] => {
  return Array.from({ length: count }, (_, i) => ({ x: baseX + (i * spacing), y }));
};

const line1Positions = generateLinePositions(LINE_1_BASE_X, LINE_1_Y, TARGET_COUNT_PER_LINE, SPACING);
const line2Positions = generateLinePositions(LINE_2_BASE_X, LINE_2_Y, TARGET_COUNT_PER_LINE, SPACING);
const line3Positions = generateLinePositions(LINE_3_BASE_X, LINE_3_Y, TARGET_COUNT_PER_LINE, SPACING);

export const PREDEFINED_TARGETS: PixelTarget[] = [
  ...line1Positions.map((pos, index) => ({
    id: crypto.randomUUID(), name: `L1-P${index + 1}`, successString: "blue" as const, x: pos.x, y: pos.y,
    line: 1,
    isMatch: false,
    detectedStatus: 'unknown',
  })),
  ...line2Positions.map((pos, index) => ({
    id: crypto.randomUUID(), name: `L2-P${index + 1}`, successString: "blue" as const, x: pos.x, y: pos.y,
    line: 2,
    isMatch: false,
    detectedStatus: 'unknown',
  })),
  ...line3Positions.map((pos, index) => ({
    id: crypto.randomUUID(), name: `L3-P${index + 1}`, successString: "red" as const, x: pos.x, y: pos.y,
    line: 3,
    isMatch: false,
    detectedStatus: 'unknown',
...
let minTargetX = Infinity, minTargetY = Infinity, maxTargetX = -Infinity, maxTargetY = -Infinity;
...
if (PREDEFINED_TARGETS.length > 0) {
  ...
  minTargetX = 0; minTargetY = 0; maxTargetX = 100; maxTargetY = 100;
}
...
export function getSpiralOffsets(gridSize: number): { dx: number; dy: number }[] {
  ...
  visited.add("0,0");
  if (gridSize > 1) {
    let curX = 0, curY = 0, dirX = 1, dirY = 0, legLength = 1, stepsInLeg = 0, turnsMade = 0;
    while (offsets.length < gridSize * gridSize) {
      curX += dirX; curY += dirY;
      const coordStr = `${curX},${curY}`;
      if (Math.abs(curX) <= halfSize && Math.abs(curY) <= halfSize) {
        if (!visited.has(coordStr)) { offsets.push({ dx: curX, dy: curY }); visited.add(coordStr); }
      } else if (legLength > gridSize * 2) { console.warn("Spiral out of bounds"); break; }
      if (offsets.length >= gridSize * gridSize) break;
      stepsInLeg++;
      if (stepsInLeg === legLength) {
        stepsInLeg = 0; const prevDirX = dirX; dirX = dirY; dirY = -prevDirX; turnsMade++;
