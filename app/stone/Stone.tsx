...
import type { PixelTarget, StoneState } from "./types";
import { classifyPixelColor, rgbToHsl } from "./utils";

function parseTargets(
	currentTargets: PixelTarget[],
	fullSourceCanvas: HTMLCanvasElement,
	fullSourceCtx: CanvasRenderingContext2D,
) {
	return currentTargets.map((target) => {
		...
		for (const offset of SPIRAL_OFFSETS_5X5) {
			...
			if (
				checkX >= 0 &&
				checkY >= 0 &&
				checkX < fullSourceCanvas.width &&
				checkY < fullSourceCanvas.height
			) {
				...
				console.error(
					`Target ${targetName}: Scan at (${checkX},${checkY}) is out of fullSourceCanvas bounds.`,
				);
			}
		}
		...
		if (
			(target.line === 1 || target.line === 2) &&
			detectedClassification === "blue"
		)
			finalStatus = "success";
		else if (target.line === 3 && detectedClassification === "red")
			finalStatus = "success";
...
export default function Stone() {
	...
	const [targets, setTargets] = useState<PixelTarget[]>(
		PREDEFINED_TARGETS.map((target) => target),
	);
	...
	useEffect(() => {
		...
		if (
			targets.some(
				(target) => target.detectedStatus === "unknown" || !target.isMatch,
			)
		) {
		...
		const line1 = targets
			.filter((target) => target.line === 1)
			.map((target) => target.detectedStatus);
		const line2 = targets
			.filter((target) => target.line === 2)
			.map((target) => target.detectedStatus);
		const line3 = targets
			.filter((target) => target.line === 3)
			.map((target) => target.detectedStatus);
	...
	const processFrame = useCallback(async () => {
		...
		const fullSourceCtx = fullSourceCanvas.getContext("2d", {
		...
		try {
			...
			const parsedTargets = parseTargets(
				targets,
				fullSourceCanvas,
				fullSourceCtx,
			);
			...
			for (const target of parsedTargets) {
				...
				for (const offset of SPIRAL_OFFSETS_5X5) {
					...
					if (
						markerXOnDisplay >= 0 &&
						markerYOnDisplay >= 0 &&
						markerXOnDisplay < displayCanvas.width &&
						markerYOnDisplay < displayCanvas.height
					) {
						if (
							markerXOnFull >= 0 &&
							markerYOnFull >= 0 &&
							markerXOnFull < fullSourceCanvas.width &&
							markerYOnFull < fullSourceCanvas.height
						) {
			...
			setTargets((currentTargets) => {
				if (
					...
					parsedTargets.some(
						(target, i) =>
							target.detectedStatus !== currentTargets[i].detectedStatus,
					)
				) {
	...
	const captureWithHiddenVideoInternal = useCallback(
		async (
			track: MediaStreamTrack,
			localFullSourceCanvas: HTMLCanvasElement,
		) => {
			...
			try {
				...
				const ctx = localFullSourceCanvas.getContext("2d", {
					willReadFrequently: true,
				});
				if (ctx) {
					ctx.drawImage(
						...
						0,
						0,
						localFullSourceCanvas.width,
						localFullSourceCanvas.height,
					);
	...
	const captureAndProcess = useCallback(
		async (sourceTrack?: MediaStreamTrack) => {
			...
			try {
				...
				const ctx = currentFullSourceCanvas.getContext("2d", {
					willReadFrequently: true,
				});
				if (ctx) {
			...
			if (videoRef.current) {
				...
				videoRef.current.onloadedmetadata = () => {
					if (
						...
						console.warn(
							"onloadedmetadata: videoRef not fully ready or stream inactive.",
						);
					}
				};
			}
	...
	useEffect(() => {
		if (isAutomating) {
			if (!mediaStream?.active) {
				console.log(
					"Automation attempt while stream is not active or lost. Stopping.",
				);
				setIsAutomating(false); // Directly set isAutomating to false
				return;
			}
